...
const sampleUrl =
    process.argv.slice(2).find((value) => value !== "--") || "https://www.youtube.com/watch?v=dQw4w9WgXcQ";
...
console.log(
    `Cobalt\t${config.cobaltApiEndpoints.length ? config.cobaltApiEndpoints.map((value) => new URL(value).host).join(", ") : "disabled"}`,
);
console.log(
    `cookies\t${config.mediaCookiesFile ? "file configured" : config.ytdlpCookiesFromBrowser ? "browser extraction configured" : "anonymous"}`,
);
